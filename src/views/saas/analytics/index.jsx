...
export default function SaaSAnalyticsView() {
  ...
  const [hotelUsage, setHotelUsage] = useState([]);
  ...
  const fetchAnalyticsData = useCallback(async () => {
    ...
    try {
      const [overviewRes, usageRes] = await Promise.all([
        ...
        SaaSAnalyticsRoute.getHotelUsage(),
      ]);
      ...
      if (usageRes?.data) {
        setHotelUsage(usageRes.data);
      }
    } catch (error) {
      console.error("Failed to fetch SaaS Analytics data:", error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const filteredHotels = hotelUsage.filter(hotel =>
  ...
  const handleExportAnalyticsReport = () => {
    const csvContent =
      "Hotel Name,Plan,Rooms,Bookings Today,System Load,Health Status\n" +
      filteredHotels
        .map(
          (h) =>
            `"${h.hotelName}","${h.plan}",${h.rooms},${h.bookingsToday},"${h.systemLoad}","${h.health}"`
        )
        .join("\n");
    ...
    a.href = url;
    a.download = `saas-analytics-report-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
  };
    ...
    <div className="space-y-8">
      ...
      <div className="flex border-b border-slate-200 dark:border-slate-800 gap-6">
        <button
          ...
          className={`pb-4 text-sm font-bold transition-all relative cursor-pointer ${activeTab === 'overview' ? 'text-indigo-600 dark:text-indigo-400 font-black' : 'text-slate-400 hover:text-slate-600'}`}
        >
        ...
        <button
          ...
          className={`pb-4 text-sm font-bold transition-all relative cursor-pointer ${activeTab === 'hotels' ? 'text-indigo-600 dark:text-indigo-400 font-black' : 'text-slate-400 hover:text-slate-600'}`}
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="glass-card p-5 rounded-lg bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex justify-between items-center">
                <div>
                  ...
                  <h3 className="text-3xl font-black text-slate-900 dark:text-white mt-1">
                    {loading ? <Loader2 className="animate-spin" size={24} /> : overview.activeHotels}
                  </h3>
              ...
              <div className="glass-card p-5 rounded-lg bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex justify-between items-center">
                <div>
                  ...
                  <h3 className="text-3xl font-black text-slate-900 dark:text-white mt-1">
                    {loading ? <Loader2 className="animate-spin" size={24} /> : `+${overview.newSignupsThisMonth}`}
                  </h3>
                  <span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10 px-2 py-0.5 rounded-full mt-2">
                ...
                <div className="h-[300px] w-full">
                  {isMounted && (
                    <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                      <AreaChart data={overview.growthData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        <defs>
              ...
              <div className="overflow-x-auto">
                {loading ? (
                  <div className="py-12 flex items-center justify-center text-slate-400 gap-2">
                    ...
                    <span>Loading hotel server load utilization...</span>
                  </div>
                ) : (
                        ...
                        <tr key={hotel.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                          ...
                          <td className="py-4">
                            <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-md uppercase ${
                              hotel.plan === 'Premium' ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400' :
                              ...
                              <div className="w-full bg-slate-100 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
                                <div
                                  className={`h-full rounded-full ${
                                    parseInt(hotel.systemLoad) > 50 ? 'bg-rose-500' :
                          ...
                          <td className="py-4">
                            <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider ${
                              hotel.health === 'Excellent' ? 'text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10' :
