...
function SidebarMenuButton({ render, isActive = false, variant = "default", size = "default", tooltip, className, asChild = false, ...props }) {
    const { isMobile, state } = useSidebar();
    const compProps = mergeProps({
        className: cn(sidebarMenuButtonVariants({ variant, size }), className),
    }, props);
    const comp = useRender({
        defaultTagName: "button",
        props: compProps,
        render: asChild ? render : (!tooltip ? render : <TooltipTrigger render={render}/>),
        state: {
